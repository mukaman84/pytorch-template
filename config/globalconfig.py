...
class Parsing_config(object):
    def __init__(self):
        super(Parsing_config, self).__init__()
        print(os.getcwd())
        self._CONFIG_PATH = 'config/globalconfig.json'
        self._check_isdir()
        # self._get_config_overall()
    ...
    # get data label config
    def _get_config_label(self):
        self._get_config_overall()
        return self.config['LABEL']

    def _get_config_mode(self,mode):
        return self._get_config_label()[mode]

    def _get_config_class(self,mode,cls):
        return self._get_config_mode(mode)[cls]



if __name__ == "__main__":
    Parsing_data = Parsing_config()
    mode_list = {'vra': 'LABEL_MOBIS_VRA', 'aa': 'LABEL_MOBIS_AA', 'vdi_ssg': 'LABEL_MOBIS_VDI_SSG', 'bdd': 'LABEL_BDD', 'cityscape': 'LABEL_CITYSCAPE'}
