...
class Inf_Normalize(object):
    ...
    def __call__(self, sample):
        img = sample['image']
        resized_img = sample['resized_img']
        lbl_path = sample['lbl_path']

        img = np.array(img).astype(np.float32)
        resized_img = np.array(resized_img).astype(np.float32)
...
class Inf_ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        resized_img = np.array(resized_img).astype(np.float32).transpose((2, 0, 1))
        resized_img = torch.from_numpy(resized_img).float()
